                            ...
                            <div class="card-body">
                                <form action="{{ route('admin.user.store') }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="username" name="username" value="{{ old('username') }}" placeholder="Enter Name">
                                        @error('username')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="role" name="role">
                                            ...
                                            @foreach(\App\Rules::cases() as $enum)
                                                <option value="{{ $enum->value }}" {{ old('role') == $enum->value ? 'selected' : '' }}>
                                                    {{ $enum->label() }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="id_employe" name="id_employe">
                                            ...
                                            @foreach($employes as $employe)
                                                <option value="{{ $employe->UserID }}" {{ old('id_employe') == $employe->UserID ? 'selected' : '' }}>
                                                    {{ $employe->UserName }}
                                                </option>
                                            @endforeach
                                        </select>
        ...
        $(document).ready(function() {
            ...
            $("#id_employe").select2({
                placeholder: function () {
                    return $(this).attr('id') === 'status' ? "SELECT A STATUS" : "CHOSE MULTIPLE SELECT";
                },
                allowClear: true,
                theme: 'bootstrap4'
            });
